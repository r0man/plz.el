...
(cl-defmethod plz-event-source-open ((source plz-buffer-event-source))
  ...
  (with-slots (buffer errors options ready-state parser) source
    (with-current-buffer (get-buffer-create buffer)
      ...
      (plz-event-source-dispatch-event source (plz-event-source-event :type "open"))
      source)))
...
(cl-defmethod plz-event-source-close ((source plz-buffer-event-source))
  ...
  (with-slots (buffer ready-state) source
    ...
    (plz-event-source-dispatch-event source (plz-event-source-event :type "close"))
    source))
...
(defun plz-http-event-source--process-filter (source process chunk)
  ...
  (when (buffer-live-p (process-buffer process))
    (with-current-buffer (process-buffer process)
      (let ((moving (= (point) (process-mark process))))
        ...
        (cond
         ((process-get process :plz-event-source-parser)
          (let ((parser (process-get process :plz-event-source-parser)))
            (with-slots (events) parser
              (while (plz-event-source-parse-line parser))
              (plz-event-source-dispatch-events source events)
              (setf events nil))))
         ((plz-event-source--response-in-buffer-p)
          (goto-char (point-min))
          (let ((response (plz--response))
                (parser (plz-event-source-parser
                         :buffer (buffer-name (current-buffer))
                         :position (plz-event-source-parser--end-of-headers))))
            (setf (oref response body) nil)
            (process-put process :plz-event-source-parser parser)
            (with-slots (ready-state response) source
              (setf ready-state 'open)
              (setf response response))
            (widen)
            (plz-event-source-dispatch-event
             source (plz-event-source-event
                     :data response
                     :type "open")))))
        (when moving
          (goto-char (process-mark process)))))))
  ...
  (with-slots (errors options process ready-state response url) source
    ...
    (let ((plz-curl-default-args (cons "--no-buffer" plz-curl-default-args)))
      ...
      (set-process-filter process (lambda (process chunk)
                                    (plz-http-event-source--process-filter source process chunk)))
      source)))
...
(defun plz-media-type:text/event-stream--event-source ()
...
(cl-defmethod plz-media-type-else ((_ plz-media-type:text/event-stream) error)
  ...
  (let ((event-source (plz-media-type:text/event-stream--event-source)))
        ...
        (setq plz-event-source--current
              (plz-event-source-open
               (plz-buffer-event-source
                :buffer (buffer-name (current-buffer))
                :handlers (oref media-type events)))))
...
(cl-defmethod plz-media-type-then ((_ plz-media-type:text/event-stream) response)
  ...
  (plz-event-source-close (plz-media-type:text/event-stream--event-source))
  response)
