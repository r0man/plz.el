...
Asynchronously stream the HTTP body in chunks using a [[https://www.gnu.org/software/emacs/manual/html_node/elisp/Filter-Functions.html][process filter]] function. The function is called repeatedly, whenever a new chunk of the HTTP body arrives.

#+BEGIN_SRC elisp :exports both :cache yes
  (plz 'get "https://httpbin.org/stream/100"
    :as `(stream
          :through ,(lambda (process chunk)
                      (message "Chunk:\n %s\n\n" chunk))))
#+END_SRC
...
+  ~plz~ :: /(method url &key headers body else finally noquery (as 'string) (then 'sync) (body-type 'text) (decode t decode-s) (connect-timeout plz-connect-timeout) (timeout plz-timeout))/
   ...
   - ~(stream :through PROCESS-FILTER)~ to asynchronously stream the HTTP response.  ~PROCESS-FILTER~ is an Emacs process filter function, and must accept two arguments: the curl process sending the request and a chunk of the HTTP body, which was just received.  
   ...
   When the HTTP response is streamed, the buffering in the curl output stream is turned off and the PROCESS-FILTER may be called multiple times, depending on the size of the HTTP body.  It is the user's responsibility to understand and process each chunk, and to construct the finalized response if necessary.  There are no guarantees regarding the chunk, such as being line-based or not.
