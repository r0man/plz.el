...
(plz-deftest plz-get-stream ()
  (seq-doseq (n (list 1 10 100 1000 10000))
    (let* ((expected-body)
           (process-1 (plz 'get (url (format "/stream/%s" n))
                        :then (lambda (body)
                                (setq expected-body body))))
           (else) (finally) (then) (through)
           (process-2 (plz 'get (url (format "/stream/%s" n))
                        :as `(stream :through ,(lambda (process chunk)
                                                 (push (list process chunk) through)))
                        :else (lambda (error)
                                (push error else))
      ...
      (should (equal '(t) finally))
      (should (equal 0 (length else)))
      (should (equal 1 (length then)))
      (seq-doseq (response then)
        (should (plz-response-p response))
        (should (eq 200 (plz-response-status response))))
      (should (>= (length through) 1))
      (seq-doseq (args through)
        (should (processp (car args)))
        (should (stringp (cadr args))))
      (should (equal expected-body (string-join (seq-map #'cl-second (reverse through)) ""))))))

(plz-deftest plz-get-stream-kill-then ()
  (let* ((else) (finally) (then) (through)
         (process (plz 'get (url "/stream/5")
                    :as `(stream :through ,(lambda (process chunk)
                                             (push (list process chunk) through)))
                    :else (lambda (error)
                            (push error else))
                    ...
                    :then (lambda (response)
                            (push response then)
                            (delete-process (plz-response-process response))))))
    (plz-test-wait process)
    (should (equal '(t) finally))
    (should (equal 1 (length else)))
    (seq-doseq (err else)
      (should (plz-error-p err))
      (should (string-match "curl process killed" (plz-error-message err)))
      (should (null (plz-error-curl-error err)))
      (should (null (plz-error-response err))))
    (should (equal 1 (length then)))
    (seq-doseq (response then)
      (should (plz-response-p response))
      (should (eq 200 (plz-response-status response))))
...
(plz-deftest plz-get-stream-kill-through ()
  (let* ((else) (finally) (then) (through)
         (process (plz 'get (url "/stream/5")
                    :as `(stream :through ,(lambda (process chunk)
                                             (push (list process chunk) through)
