...
(ert-deftest event-source-parse-mutli-line-event ()
  (with-temp-buffer
    (let ((event-1 (event-source-event
          ...
          (event-2 (event-source-event
                    ...
                    :origin (buffer-name)))
          (parser (event-source-parser :buffer (buffer-name))))
      (with-slots (events) parser
        (event-source-parser-insert parser "data: This is the first message.\n")
        (should (null events))
        (event-source-parser-insert parser "\n")
        (should (equal (list event-1) events))
        (event-source-parser-insert parser "data: This is the second message, it\n")
        (should (equal (list event-1) events))
        (event-source-parser-insert parser "data: has two lines.\n")
        (should (equal (list event-1) events))
        (event-source-parser-insert parser "\n")
        (should (equal (list event-2 event-1) events))
        (event-source-parser-insert parser "data: This is the third message.\n")
        (should (equal (list event-2 event-1) events))
        (should (equal "" (buffer-string)))))))

(ert-deftest event-source-parse-event-types ()
  (with-temp-buffer
    (let ((event-1 (event-source-event
                    :type "add"
                    :data "73857293"
          ...
          (parser (event-source-parser :buffer (buffer-name))))
      (with-slots (events) parser
        (event-source-parser-insert parser "event: add\n")
        (should (null events))
        (event-source-parser-insert parser "data: 73857293\n")
        (should (null events))
        (event-source-parser-insert parser "\n")
        (should (equal (list event-1) events))
        (event-source-parser-insert parser "event: remove\n")
        (should (equal (list event-1) events))
        (event-source-parser-insert parser "data: 2153\n")
        (should (equal (list event-1) events))
        (event-source-parser-insert parser "\n")
        (should (equal (list event-2 event-1) events))
        (event-source-parser-insert parser "event: add\n")
        (should (equal (list event-2 event-1) events))
        (event-source-parser-insert parser "data: 113411\n")
        (should (equal (list event-2 event-1) events))
        (should (equal "" (buffer-string)))))))

(ert-deftest event-source-parse-stock-ticker ()
  (with-temp-buffer
    (let ((event-1 (event-source-event
                    :data "YHOO\n+2\n10"
                    :origin (buffer-name)))
          (parser (event-source-parser :buffer (buffer-name))))
      (with-slots (events) parser
        (event-source-parser-insert parser "data: YHOO\n")
        (should (null events))
        (event-source-parser-insert parser "data: +2\n")
        (should (null events))
        (event-source-parser-insert parser "data: 10\n")
        (should (null events))
        (event-source-parser-insert parser "\n")
        (should (equal (list event-1) events))
        (should (equal "" (buffer-string)))))))

(ert-deftest event-source-parse-four-blocks ()
  (with-temp-buffer
    (let ((event-1 (event-source-event
                    :data "first event"
                    :last-event-id "1"
          ...
          (event-2 (event-source-event
                    :data "second event"
                    :origin (buffer-name)))
          (event-3 (event-source-event
                    :data " third event"
                    :origin (buffer-name)))
          (parser (event-source-parser :buffer (buffer-name))))
      (with-slots (events) parser
        (event-source-parser-insert parser ": test stream\n")
        (should (null events))
        (event-source-parser-insert parser "\n")
        (should (null events))
        (event-source-parser-insert parser "data: first event\n")
        (should (null events))
        (event-source-parser-insert parser "id: 1\n")
        (should (null events))
        (event-source-parser-insert parser "\n")
        (should (equal (list event-1) events))
        (event-source-parser-insert parser "data:second event\n")
        (should (equal (list event-1) events))
        (event-source-parser-insert parser "id\n")
        (should (equal (list event-1) events))
        (event-source-parser-insert parser "\n")
        (should (equal (list event-2 event-1) events))
        (event-source-parser-insert parser "data:  third event\n")
        (should (equal (list event-2 event-1) events))
        (event-source-parser-insert parser "\n")
        (should (equal (list event-3 event-2 event-1) events))
        (should (equal "" (buffer-string)))))))

(ert-deftest event-source-parse-two-events ()
  (with-temp-buffer
    (let ((event-1 (event-source-event
                    :data ""
                    :origin (buffer-name)))
          ...
          (parser (event-source-parser :buffer (buffer-name))))
      (with-slots (events) parser
        (event-source-parser-insert parser "data\n")
        (should (null events))
        (event-source-parser-insert parser "\n")
        (should (equal (list event-1) events))
        (event-source-parser-insert parser "data\n")
...
(ert-deftest event-source-parse-two-identical-events ()
  (with-temp-buffer
    (let ((event-1 (event-source-event
                    :data "test"
                    :origin (buffer-name)))
          (parser (event-source-parser :buffer (buffer-name))))
      (with-slots (events) parser
        (event-source-parser-insert parser "data:test\n")
        (should (null events))
        (event-source-parser-insert parser "\n")
        (should (equal (list event-1) events))
        (event-source-parser-insert parser "data: test\n")
    ...
    (let ((events)
          (source (plz-event-source
                   ...
                   :options `((body . ,(json-encode
                              ...
                              (method . post)))))
      (event-source-add-listener source "open"
                                 (lambda (source event)
                                   (should (cl-typep source 'event-source))
                                   (should (cl-typep event 'event-source-event))
                                   (should (equal "open" (event-source-event-type event)))
                                   (push event events)))
      (event-source-add-listener source "message"
                                 (lambda (source event)
                                   (should (cl-typep source 'event-source))
                                   (should (cl-typep event 'event-source-event))
                                   (should (equal "message" (event-source-event-type event)))
                                   (push event events)))
      (event-source-add-listener source "close"
                                 (lambda (source event)
                                   (should (cl-typep source 'event-source))
                                   (should (cl-typep event 'event-source-event))
                                   (should (equal "close" (event-source-event-type event)))
      ...
      (with-slots (ready-state url) source
        ...
        (should (equal (list "open" "message" "close")
                       (cl-remove-duplicates
                        (seq-map #'event-source-event-type
                                 (reverse events)))))
...
;;; test-event-source.el ends here
